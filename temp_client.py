...
def update_CGU (args, model, mat_list, threshold, task_id, feature_list=[], importance_list=[]):
...
class Client:
    ...
    def train_first_task(self, args, xtrain,ytrain,xvalid,yvalid,xtest,ytest, task_id, device, threshold, c_id ,old_model_list, g_round, lr, best_loss):
        ...
        self.feature_list, self.importance_list = update_CGU(args, self.model, mat_list, threshold, task_id, feature_list, importance_list)
        return
    ...
    def train_next_task(self, args, xtrain,ytrain,xvalid,yvalid,xtest,ytest, task_id, device, threshold, c_id ,old_model_list, g_round, lr, best_loss):
        ...
        # Memory and Importance Update 
        mat_list = get_representation_matrix (self.model, device, xtrain, ytrain)
        self.feature_list, self.importance_list = update_CGU (args, self.model, mat_list, threshold, task_id, self.feature_list, self.importance_list)
        return
...
def fed_main(args):
    ...
    data_set, taskcla = cf100.get_cifar(args.increment_mode, seed=args.seed, 
...
if __name__ == "__main__":
    # Training parameters
    parser = argparse.ArgumentParser(description='10-split CIFAR-100 with SGP')
